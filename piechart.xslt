...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    <xsl:template match="Dataset[starts-with(@name,'PieChart')]" mode="generate_body">
        <h1>
<xsl:value-of select="translate(substring-after(@name, 'PieChart_'),'_',' ')"/>
</h1>
...
<xsl:text>() {
                ...
                <xsl:text>,0.6);var options = {fontName:'Arial',tooltip: { textStyle: { fontName: 'Arial', fontSize: 18, bold:false }},is3D: true};var piechart = new google.visualization.PieChart(document.getElementById('</xsl:text>
<xsl:value-of select="@name"/>
<xsl:text>'));
