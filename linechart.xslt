...
--><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    <xsl:template match="Dataset[starts-with(@name,'LineChart')]" mode="generate_body">
        <h1>
         <xsl:value-of select="translate(substring-after(@name, 'LineChart_'),'_',' ')"/>
      </h1>
	     <head>
    ...
    <xsl:template match="Dataset[starts-with(@name,'LineChart')]" mode="generate_script">
      ...
      <xsl:value-of select="@name"/>
	     <xsl:text>() {
                ...
                <xsl:text>,0.6);var linechart = new google.visualization.LineChart(document.getElementById('</xsl:text>
      <xsl:value-of select="@name"/>           
      <xsl:text>'));
